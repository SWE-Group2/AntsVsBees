...
/**
 * A ZombieBee is a dangerous bee that can attack up to 3 ants at once! Unlike a normal bee that only stings the ant
 * directly in its way, a ZombieBee attacks the ant in its current place AND the ants in the next 2 places ahead of it.
 *
 * <p>
...
public class ZombieBee extends Bee {
    ...
    /**
     * ZombieBee's action - attacks up to 3 ants at once! It stings the ant in its current place AND ants in the next 2
     * places ahead. If no ants are found in range, it moves forward like a normal bee.
     *
     * <p>
     * Pre-condition: colony must not be null Post-condition: up to 3 ants take damage, or zombie bee moves forward
     *
     * @param colony
     *            The ant colony
     */
    ...
    public void action(AntColony colony) {
        ...
        for (int i = 0; i < ATTACK_RANGE && current != null; i++) {
            ...
            if (ant != null) {
                ...
                sting(ant);
                attackCount++;
                System.out.println("ZombieBee attacked " + ant + " at " + current + " (" + attackCount + "/"
                        + ATTACK_RANGE + " attacks)");
            }
