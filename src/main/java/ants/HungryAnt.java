...
/**
 * A HungryAnt is an ant that eats bees! It randomly picks a bee from its location and instantly kills it. However,
 * after eating a bee, it needs 3 turns to digest before it can eat again.
 *
 * <p>
 * Food Cost: 4, Armor: 1
 *
...
public class HungryAnt extends Ant {
    ...
    /**
     * Creates a new HungryAnt. Starts with 0 digestion turns (ready to eat straight away). Post-condition: food cost is
     * 4, armor is 1, digesting is 0
     */
    public HungryAnt() {
        super(1); // armor = 1
        this.foodCost = 4;
    ...
    /**
     * Each turn, the HungryAnt either: - Eats a random bee if it is not digesting - Counts down its digestion timer if
     * it is still digesting
     *
     * <p>
     * Pre-condition: colony must not be null Post-condition: either a bee is eaten or digestion timer decreases by 1
     *
     * @param colony
     *            The ant colony (used to check game state)
     */
    @Override
    public void action(AntColony colony) {
        ...
        if (turnsDigesting > 0) {
            ...
            if (target != null) {
                ...
                System.out.println("HungryAnt ate " + target + "! Now digesting for " + DIGESTION_TIME + " turns.");
