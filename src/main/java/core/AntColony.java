...
public class AntColony {
   ...
   * @param moatFrequency The frequency of which moats (water areas) appear. 0 means that there are
   *     no moats
    ...
    for (int tunnel = 0; tunnel < numTunnels; tunnel++) {
      ...
      for (int step = 0; step < tunnelLength; step++) {
        ...
        if (step == moatIndex) {
          curr =
              new WaterPlace(
                  "tunnel[" + tunnel + "-" + step + "]", prev); // create water place with an
          // exit that
          // is
          // the previous spot
        } else {
          curr =
              new Place(
                  "tunnel[" + tunnel + "-" + step + "]",
                  prev); // create new place with an exit that
          // is
          // the previous spot
        }
      ...
      beeEntrances.add(
          curr); // current place is last item in the tunnel, so mark that it is a bee entrance
    } // loop to next tunnel
  }

  /**
   * Returns an array of Places in this colony. Places are ordered by tunnel, with each tunnel's
   * places listed start to end.
   *
  ...
  /**
   * Places the given ant in the given tunnel IF there is enough available food. Otherwise has no
   * effect
   *
  ...
  public void deployAnt(Place place, Ant ant) {
    ...
    if (place.canAddInsect(ant) && this.food >= ant.getFoodCost()) {
      ...
      place.addInsect(ant);
    } else System.out.println("Not enough food remains to place " + ant);
  }
  ...
  public ArrayList<Ant> getAllAnts() {
    ...
    for (Place p : places) {
      if (p.getAnt() != null) ants.add(p.getAnt());
    }
    return ants;
  }
  ...
  public ArrayList<Bee> getAllBees() {
    ...
    for (Place p : places) {
      for (Bee b : p.getBees()) bees.add(b);
    }
    return bees;
  }
