...
public class Bee extends Insect {
  ...
  public boolean isBlocked() {
    Ant ant = this.place.getAnt();
    return ant != null && ant.blocksBees();
  }

  /**
   * A bee's action is to sting the Ant that blocks its exit if it is blocked, otherwise it moves to
   * the exit of its current place.
   */
  ...
  public void reduceEffects() {
    if (this.slowedTurns > 0) this.slowedTurns -= 1;
