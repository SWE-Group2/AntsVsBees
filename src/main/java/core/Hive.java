...
public class Hive extends Place {
  ...
  private Map<Integer, Bee[]>
      waves; // a mapping from attack times to the list of bees that will charge in
  ...
  public Bee[] invade(AntColony colony, int currentTime) {
    ...
    if (wave == null) return new Bee[0]; // return empty set if no bees attacking now
  ...
  public static Hive makeFullHive() {
    Hive hive = new Hive(3);
    hive.addWave(2, 1);
    for (int i = 3; i < 15; i += 2) hive.addWave(i, 1);
    hive.addWave(15, 8);
    return hive;
  }
  ...
  public static Hive makeInsaneHive() {
    ...
    hive.addWave(1, 2);
    for (int i = 3; i < 15; i++) hive.addWave(i, 1);
    hive.addWave(15, 20);
    return hive;
  }
}
