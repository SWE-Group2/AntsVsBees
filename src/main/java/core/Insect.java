...
public abstract class Insect {
  ...
  public Insect(int armor, Place place) {
    if (armor <= 0) throw new IllegalArgumentException("Cannot create an insect with armor of 0");
    this.armor = armor;
    this.place = place;
  }
  ...
  /**
   * Has the insect move out of its current location. Abstract in case the insect takes action when
   * it leaves
   */
